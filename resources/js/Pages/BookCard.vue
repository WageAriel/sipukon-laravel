<template>
    <article
        ...
        <div class="relative rounded-t-lg overflow-hidden bg-gray-100">
            <img :src="`/storage/book_covers/${data.cover_image}`" alt="Book Cover"
        ...
        <div class="p-4 flex flex-col">
            ...
            <p class="text-sm text-red-600 mt-1"> {{ data.status_buku }}</p>
        </div>
        ...
        <div v-if="showModal" class="fixed inset-0 bg-gray-900 bg-opacity-50 flex items-center justify-center z-50">
            <div class="bg-white rounded-lg shadow-lg w-full max-w-md p-6">
                ...
                <div class="flex justify-end space-x-4 mt-4">
                    <!-- Button Pinjam -->
                    <button v-if="data.status_buku === 'tersedia'" @click.stop="redirectToForm(data.title)"
                        class="bg-sky-500 text-white px-4 py-2 rounded-md hover:bg-sky-600">
                        Pinjam
                    </button>

                    <!-- Button Kembali -->
                    <button @click.stop="closeModal" class="bg-gray-300 px-4 py-2 rounded-md hover:bg-gray-400">
                        Kembali
                    </button>
                </div>
            </div>
