<template>
  <header class="flex overflow-hidden flex-wrap gap-10 px-9 py-3.5 whitespace-nowrap bg-white bg-opacity-0 max-md:px-5 justify-between items-center">
    ...
    <div class="flex items-center">
      <img
        loading="lazy"
        src="../image/logo-uns-besar-biru 1.png"
        alt="Company logo"
        class="object-contain shrink-0 self-stretch max-w-full aspect-[2.11] w-[150px] md:w-[207px]"
      />
    </div>
    ...
    <div class="hidden md:flex gap-4 my-auto text-sm font-extrabold text-center text-white items-center">
      <img
        loading="lazy"
        src="https://cdn.builder.io/api/v1/image/assets/d32ba014fd2f43b69edb017a43e6b422/70ae46c9b276bc3e2a70e0880c2c18f82ac4e26f8512fa945770e15590e71200?apiKey=d32ba014fd2f43b69edb017a43e6b422&"
        alt="User avatar"
        class="object-contain shrink-0 rounded-3xl aspect-square w-[40px] md:w-[60px]"
      />
      <Link
        v-if="!auth.user"
        :href="route('login')"
        class="overflow-hidden gap-2.5 px-6 py-3 bg-sky-500 rounded-xl"
      >
        Login
      </Link>
      <button
        v-else
        @click="logout"
        class="overflow-hidden gap-2.5 px-6 py-3 bg-red-500 rounded-xl"
      >
        Logout
      </button>
    </div>
    ...
    <aside
      v-if="isSidebarOpen"
      class="fixed top-0 right-0 w-64 h-full bg-white shadow-lg z-50 transform transition-transform translate-x-0 md:hidden"
    >
      ...
      <div class="flex flex-col items-center gap-4 mt-4 px-8">
        <img
          ...
          src="https://cdn.builder.io/api/v1/image/assets/d32ba014fd2f43b69edb017a43e6b422/70ae46c9b276bc3e2a70e0880c2c18f82ac4e26f8512fa945770e15590e71200?apiKey=d32ba014fd2f43b69edb017a43e6b422&"
          alt="User avatar"
          class="object-contain shrink-0 rounded-3xl aspect-square w-16"
        />
        <Link
          v-if="!auth.user"
          :href="route('login')"
          class="overflow-hidden px-6 py-3 bg-sky-500 text-white rounded-xl w-full text-center"
          @click="toggleSidebar"
        >
          Login
        </Link>
        <button
          v-else
          @click="logout"
          class="overflow-hidden px-6 py-3 bg-red-500 text-white rounded-xl w-full"
        >
          Logout
        </button>
      </div>
